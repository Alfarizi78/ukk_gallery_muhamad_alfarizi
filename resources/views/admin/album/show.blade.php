<x-app-layout>
            ...
            <div class="flex flex-wrap gap-2 w-full sm:w-auto">
                @if($canCreateSubAlbum)
                    <button onclick="toggleSubAlbumForm()" 
                            class="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 bg-gradient-to-r from-purple-500 to-indigo-600 text-white font-bold rounded-lg transition-all duration-300 hover:scale-105">
                ...
                <a href="{{ route('admin.photo.create', ['album_id' => $album->album_id]) }}" 
                   class="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 bg-gradient-to-r from-green-500 to-emerald-600 text-white font-bold rounded-lg transition-all duration-300 hover:scale-105">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="p-4 sm:p-6">
                    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                        ...
                        <div class="flex flex-wrap gap-2 w-full sm:w-auto">
                            ...
                            <a href="{{ route('admin.photo.create', ['album_id' => $album->album_id]) }}" 
                               class="w-full sm:w-auto inline-flex items-center justify-center px-4 py-2 bg-gradient-to-r from-green-500 to-emerald-600 rounded-lg text-white transition-all duration-300 hover:scale-105">
                                <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                                </svg>
                                Add Photos
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                                @foreach($album->photos as $photo)
                                    <div class="relative group">
                                        ...
                                        <!-- Photo Card -->
                                        <div class="relative aspect-square overflow-hidden rounded-lg shadow-lg cursor-pointer bg-gray-200 dark:bg-gray-700"
                                            ...
                                            <!-- Hover Overlay -->
                                            <div class="absolute inset-0 bg-gradient-to-t from-black/70 via-black/50 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300">
    ...
    <script>
        ...
        function goBack() {
            const currentDepth = {{ $albumDepth }};
            
            if (currentDepth > 0) {
                // If has parent_id, go back to parent album
                const parentId = {{ $album->parent_id ?? 'null' }};
                if (parentId) {
                    window.location.href = "{{ route('admin.album.show', '') }}/" + parentId;
                } else {
                    // If no parent_id, go back to index
                    window.location.href = "{{ route('admin.album.index') }}";
                }
            } else {
                // If at level 0 (main album), go back to index
                window.location.href = "{{ route('admin.album.index') }}";
            }
        }

        function toggleSubAlbumForm() {
